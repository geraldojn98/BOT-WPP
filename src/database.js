...
// =====================================================================
// BANCO GLOBAL — só autenticação (users/sessions). Cada usuário tem seu
// próprio arquivo de banco separado (data/users/<id>.db, ver mais abaixo)
// pra tudo que é dado de verdade (produtos, grupos, perfis, histórico...)
// — isolamento por arquivo físico, não por coluna/WHERE, então não tem
// como um bug de query vazar dado de um usuário pro outro.
// =====================================================================
const AUTH_DB_PATH = path.join(DATA_DIR, 'bot.db');
...
// tabela que já existe, então isso cobre quem já tinha o banco criado.
try {
  authDb.exec('ALTER TABLE webhook_tokens ADD COLUMN user_id INTEGER');
} catch (_) { /* coluna já existe (banco novo, criado já com o schema atual) */ }
...
try {
  const hasUsers = authDb.prepare('SELECT COUNT(*) as c FROM users').get().c > 0;
  if (!hasUsers && process.env.PANEL_USER && process.env.PANEL_PASS) {
    const hash = bcrypt.hashSync(process.env.PANEL_PASS, 10);
    authDb.prepare('INSERT INTO users (username, password_hash) VALUES (?, ?)').run(process.env.PANEL_USER, hash);
...
// Migração única: tokens de webhook criados antes do multiusuário não tinham
// dono — associa todos ao usuário criado a partir de PANEL_USER (o único que
// poderia tê-los criado antes de existir login).
try {
  if (process.env.PANEL_USER) {
    const admin = authDb.prepare('SELECT id FROM users WHERE username = ?').get(process.env.PANEL_USER);
    if (admin) {
      authDb.prepare('UPDATE webhook_tokens SET user_id = ? WHERE user_id IS NULL').run(admin.id);
    }
  }
} catch (_) { /* já migrado ou erro não-crítico */ }
...
// valor do token, antes de saber de qual usuário é, então ficam no banco de
// autenticação em vez do banco por usuário) =====
function getAllWebhookTokensForUser(userId) {
  return authDb.prepare('SELECT * FROM webhook_tokens WHERE user_id = ? ORDER BY created_at DESC').all(userId);
}
function addWebhookToken(userId, name, token) {
  return authDb.prepare('INSERT INTO webhook_tokens (user_id, name, token) VALUES (?, ?, ?)').run(userId, name, token);
}
function deleteWebhookToken(id) {
  return authDb.prepare('DELETE FROM webhook_tokens WHERE id = ?').run(id);
}
function toggleWebhookToken(id, active) {
  return authDb.prepare('UPDATE webhook_tokens SET active = ? WHERE id = ?').run(active, id);
}
function getWebhookTokenByValue(token) {
  return authDb.prepare('SELECT * FROM webhook_tokens WHERE token = ?').get(token);
}
function getWebhookTokenById(id) {
  return authDb.prepare('SELECT * FROM webhook_tokens WHERE id = ?').get(id);
}
function touchWebhookTokenUsage(id) {
  return authDb.prepare("UPDATE webhook_tokens SET last_used_at = datetime('now') WHERE id = ?").run(id);
}

// =====================================================================
// BANCO POR USUÁRIO — tudo que é dado de verdade (produtos, histórico,
// configurações, perfis de postagem, grupos, campanhas, Instagram...).
// Cada usuário tem seu próprio arquivo data/users/<id>.db, criado sob
// demanda na primeira vez que é acessado (mesmo schema/migrações que já
// existiam, só que rodando por-arquivo em vez de uma vez só globalmente).
// =====================================================================
...
function createUserDb(userId) {
  ...
  return {
    ...
    // Webhooks (assinaturas de saída — os tokens de entrada agora são globais, ver getWebhookTokenByValue etc.)
...
function getUserDb(userId) {
  if (!userDbCache.has(userId)) userDbCache.set(userId, createUserDb(userId));
  return userDbCache.get(userId);
}

// Migração única: se o usuário criado a partir de PANEL_USER/PANEL_PASS ainda não
// tem seu próprio arquivo de banco, e o bot.db antigo (global) tem as tabelas de
// dados de antes do multiusuário, copia o arquivo inteiro pra virar o banco desse
// usuário — preserva 100% do histórico/produtos/perfis/grupos de quem já usava o
// bot, sem precisar reconstruir nada tabela por tabela.
try {
  if (process.env.PANEL_USER) {
    const admin = getUserByUsername(process.env.PANEL_USER);
    if (admin) {
      const adminDbPath = path.join(USERS_DIR, `${admin.id}.db`);
      if (!fs.existsSync(adminDbPath)) {
        const hasLegacyData = authDb.prepare(
          "SELECT COUNT(*) as c FROM sqlite_master WHERE type='table' AND name='products'"
        ).get().c > 0;
        if (hasLegacyData) {
          fs.copyFileSync(AUTH_DB_PATH, adminDbPath);
          userDbCache.delete(admin.id); // força reabrir do arquivo recém-copiado na próxima chamada
          console.log(`[DB] Dados existentes migrados para o usuário "${admin.username}".`);
        }
      }
    }
  }
} catch (err) {
  console.error('[DB] Erro na migração de dados legados pro banco por usuário:', err.message);
}
